...
public final class MCSessionManager: NSObject,SessionHelperProtocol{
    ...
    internal func onReceivedTextData(_ callback:@escaping(Data, MCPeerID) -> Void) {
...
//            }
    ...
    internal func sendImage(imageData: Data?, sendMode : MCSessionSendDataMode, target: [MCPeerID]) throws {
        guard let weakSession = self.session else {return}
        guard self.serviceProtocol == .textAndVideo else { return }
        
        try streamHelper.send(session: weakSession, data: imageData ?? Data(), sendMode: sendMode, targets: target)
    }
