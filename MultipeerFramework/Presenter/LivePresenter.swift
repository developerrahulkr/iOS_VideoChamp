...
open class LivePresenter: NSObject {
    //models
    ...
    public var cap : AVCaptureSession!
    ...
    public init(mcSessionManager: MCSessionManager, sendVideoInterval: TimeInterval,
                ...
                cameraInitPosition: AVCaptureDevice.Position = .back,
    ...
    public func bindReceivedCallbacks(gotImage:@escaping(UIImage?, MCPeerID) -> Void,
                                      ...
                                      gotTextMessage: ((Data, MCPeerID)->Void)?=nil) {
    ...
    public func send(text: Data, sendMode: MCSessionSendDataMode) throws {
        guard let _targetPeerID = self.targetPeerID else { return }
//        let data = text.data(using: .utf8)!
        try mcSessionManager.send(data: text, sendMode: sendMode, targets: [_targetPeerID])
    }
    
    public func sendText(text: String, sendMode: MCSessionSendDataMode) throws {
        guard let _targetPeerID = self.targetPeerID else { return }
        let data = text.data(using: .utf8)!
    ...
    private func setUpCamera(initPosition: AVCaptureDevice.Position,
        ...
        cap = mediaPresenter.captureSession
    }
